...
void GameLoop(PlayerInfo &player, const Conversation &conversation, const string &testToRunName, bool debugMode)
{
    ...
    while (!menuPanels.IsDone())
    {
        ...
        if (runningTest && dataFinishedLoading)
        {
            ...
            // Skip drawing 29 out of every 30 in-flight frames during testing to speedup testing (unless debug mode is
            // set). We don't skip UI-frames to ensure we test the UI code more.
